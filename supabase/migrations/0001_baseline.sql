-- =====================================================================
-- 0001 · ESQUEMA-BASE REPRODUZIVEL  (baseline)
--
-- Este arquivo sozinho, num banco Supabase VAZIO, reconstroi o FPV Campo
-- exatamente como ele esta em PRODUCAO em 28/07/2026 — 9 tabelas, os
-- gatilhos, a RLS e o realtime. E o substituto dos 14 .sql soltos na
-- raiz do repo (ver seguranca/08-migrations-PLANO.md).
--
...
select setval('public.seq_fict',
              greatest(coalesce((select max(numero_fict) from public.os_campo), 76), 76));
...
-- 5.2 saida_material — DELETE so gestao + Joao
...
declare
  ...
  gestao text := $g$auth.jwt() ->> 'email' in
    ('lucas@fpv.app','rafael@fpv.app','nicolas@fpv.app','renan@fpv.app','edmar@fpv.app','joao@fpv.app')$g$;
begin
